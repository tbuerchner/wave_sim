...
for j_element in range(n_elements[1]):
    for i_element in range(n_elements[0]):
        ...
        for igauss, (r, w0) in enumerate(zip(rValues, gll_weights)):
            for jgauss, (s, w1) in enumerate(zip(sValues, gll_weights)):
                weight = w0 * w1 * dx / (r1 - r0) * dy / (s1 - s0)

                # Compute tensor product and derivatives w.r.t. x and y
                N = np.outer(shapes_s_diff0[jgauss], shapes_r_diff0[igauss]).ravel()

                # Add contributions
                Me += np.outer(N, N) * rho * weight
        ...
        for igauss, (r, w0) in enumerate(zip(rValues, gl_weights)):
            for jgauss, (s, w1) in enumerate(zip(sValues, gl_weights)):
                ...
                # Add contributions
                Ke += (np.outer(dNdx, dNdx) + np.outer(dNdy, dNdy)) * rho * c * c * weight
                Fe += N * fx(x, y) * weight
...
def animate(i):
    ax.clear()
    Z = np.reshape(u[i], (n_dof_direction[0], n_dof_direction[1]))
...
anim = animation.FuncAnimation(fig, animate, range(0, n_steps, 4), interval=2)

plt.show()

# Plot last state
Z = np.reshape(u[-1], (n_dof_direction[0], n_dof_direction[1]))
