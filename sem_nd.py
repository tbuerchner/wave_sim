...
# Spatial dimension
dim = 4
print(f"Setting up {dim}D SEM problem...")

# domain
origin = np.array([0.0]*dim)
length = np.array([1.0] + [0.1]*(dim-1))
print(f"Domain origin: {origin}, length: {length}")

# mesh
n_elements = np.array([20] + [2]*(dim-1))
polynomial_degree = np.array([4] + [2]*(dim-1))
print(f"Number of elements: {n_elements}, Polynomial degree: {polynomial_degree}")
mesh_nd = mesh.ndRectangle(dim=dim, origin=origin, lengths=length, n_elements=n_elements, polynomial_degree=polynomial_degree)
...
n_entries_assembly = mesh_nd.n_elements_total * mesh_nd.n_dofs_element ** 2
...
f_ex = np.zeros(mesh_nd.n_dofs_global)
ic_0 = np.zeros(mesh_nd.n_dofs_global)
ic_1 = np.zeros(mesh_nd.n_dofs_global)


for element_indices in tqdm(itertools.product(*[range(n) for n in np.flip(n_elements)]), 
                     total=mesh_nd.n_elements_total, desc="Processing elements"):
    element_indices = element_indices[::-1]
    tqdm.write(f"Current element indices: {element_indices}")

    # Local element matrices 
    proje = np.zeros((mesh_nd.n_dofs_element, mesh_nd.n_dofs_element))
    Me = np.zeros((mesh_nd.n_dofs_element, mesh_nd.n_dofs_element))
    Ke = np.zeros((mesh_nd.n_dofs_element, mesh_nd.n_dofs_element))

    # Local element vectors
    f_exe = np.zeros((mesh_nd.n_dofs_element,))
    ic_0e = np.zeros((mesh_nd.n_dofs_element,))
    ic_m1e = np.zeros((mesh_nd.n_dofs_element,))

    # integrate over whole element 
    # (structure expandable for higher dimensions and cut cells)
    (r0, r1) = ([-1.0] * dim, [1.0] * dim)
    ...
    for integration_indices in itertools.product(*[range(n) for n in np.flip(polynomial_degree + 1)]):
        ...
        weight1 = 1.0
        for i_d in range(dim):
            weight1 *= int_weights_rule1[i_d][igauss[i_d]] * mesh_nd.dx[i_d] / (r1[i_d] - r0[i_d])

        # Add contributions
        N = shapes_r_rule1_0[0][igauss[0]]
        for i_d in range(1, dim):
            N = np.outer(shapes_r_rule1_0[i_d][igauss[i_d]], N).ravel()
        Me += weight1 * mat.density * np.outer(N, N)

        # scale weight for physical element
        weight2 = 1.0
        for i_d in range(dim):
            weight2 *= int_weights_rule2[i_d][igauss[i_d]] * mesh_nd.dx[i_d] / (r1[i_d] - r0[i_d])
        # Add contributions
        N0 = shapes_r_rule2_0[0][igauss[0]]
        N1_list = []
        for i_d in range(dim):
            if i_d == 0:
                N1_list.append(shapes_r_rule2_1[0][igauss[0]] * (2.0 / mesh_nd.dx[0]))
            else:
                N1_list.append(shapes_r_rule2_0[0][igauss[0]])
        for i_d in range(1, dim):
            N0 = np.outer(shapes_r_rule2_0[i_d][igauss[i_d]], N0).ravel()
            for j_d in range(dim):
                if j_d == i_d:
                    N1_list[j_d] = np.outer(shapes_r_rule2_1[i_d][igauss[i_d]] * (2.0 / mesh_nd.dx[i_d]), N1_list[j_d]).ravel()
                else:
                    N1_list[j_d] = np.outer(shapes_r_rule2_0[i_d][igauss[i_d]], N1_list[j_d]).ravel()

        Ke += weight2 * mat.density * mat.wave_speed ** 2 * sum(np.outer(N1_list[i_d], N1_list[i_d]) for i_d in range(dim))
        proje += weight2 * np.outer(N0, N0)
        x_phys = mesh_nd.map_to_physical(element_indices, [r_values_rule2[i_d][igauss[i_d]] for i_d in range(dim)])
    ...
    lm = mesh_nd.get_location_map(element_indices)
    # Repeat locationMaps as colums/rows linearize the result to obtain matrix entry coordinates
    element_slice = mesh_nd.get_element_slice(element_indices)

    row[element_slice] = np.broadcast_to(lm, (mesh_nd.n_dofs_element, mesh_nd.n_dofs_element)).ravel()
    col[element_slice] = np.broadcast_to(lm, (mesh_nd.n_dofs_element, mesh_nd.n_dofs_element)).T.ravel()
...
M = scipy.sparse.coo_matrix((data_M, (row, col)), shape=(mesh_nd.n_dofs_global, mesh_nd.n_dofs_global)).tocsc()
K = scipy.sparse.coo_matrix((data_K, (row, col)), shape=(mesh_nd.n_dofs_global, mesh_nd.n_dofs_global)).tocsc()
proj = scipy.sparse.coo_matrix((data_proj, (row, col)), shape=(mesh_nd.n_dofs_global, mesh_nd.n_dofs_global)).tocsc()
...
u = np.zeros((n_steps + 1, mesh_nd.n_dofs_global))
...
n_z_slices = 2 # n_elements[2] * polynomial_degree[2] + 1
fig, axes = plt.subplots(n_z_slices,1)
line, = axes[0].plot([], [], lw=2)
for i_x, ax in enumerate(axes):
    ax.set_xlim(origin[0], origin[0] + length[0])
    ax.set_ylim(origin[1], origin[1] + length[1])
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_title(f'2D SEM Wave Propagation ({i_x}th slice in z)')
    ax.set_aspect('equal', adjustable='box')
def init():
    line.set_data([], [])
    return line,
x = np.empty(mesh_nd.n_dofs_global_directions[0])
y = np.empty(mesh_nd.n_dofs_global_directions[1])
for i_el_x in range(n_elements[0]):
    for i_p_x, point_x in enumerate(gll_nodes[0]):
        x[i_el_x * (polynomial_degree[0]) + i_p_x] = mesh_nd.map_to_physical([i_el_x, 0]+[0]*(dim-2), [point_x, 0.0]+[0.0]*(dim-2))[0]
for i_el_y in range(n_elements[1]):
    for i_p_y, point_y in enumerate(gll_nodes[1]):
        y[i_el_y * (polynomial_degree[1]) + i_p_y] = mesh_nd.map_to_physical([0, i_el_y]+[0]*(dim-2), [0.0, point_y]+[0.0]*(dim-2))[1]
X, Y = np.meshgrid(x, y, indexing='ij')
Xe, Ye = np.meshgrid(np.linspace(0, length[0], n_elements[0] + 1),
                     np.linspace(0, length[1], n_elements[1] + 1), indexing='ij')
def animate(i):
    for i_x, ax in enumerate(axes):
        size_xy = np.prod(mesh_nd.n_dofs_global_directions[0:2])
        Z = np.reshape(u[i, i_x*size_xy:(i_x+1)*size_xy], (mesh_nd.n_dofs_global_directions[1], mesh_nd.n_dofs_global_directions[0])).T
        ax.clear()
